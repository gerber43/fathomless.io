        ...
        <script>
            ...
            function clickTile(tileId) {
                ...
                if (document.getElementById(tileId).dataset.manhattan == 1 && !attacking) {
                    ...
                    directionHandler(keyBinds["Movement"][(index != -1)?index:3])
                }
                if (attacking) {
                    sendRequest("?sendAttack="+encodeURIComponent(tileId));
                    toggleAttack();
                }
            }
            ...
            function moveObject(gameObject,x,y) {
                ...
                gameObject.style.transform = (x != 0 || y != 0)?"translate("+x+"px, "+y+"px)":"";
            }
            function directionHandler(keycode) {
                if (!disableMovement) {
                    sendRequest("?sendDirection="+((keyBinds["Movement"].indexOf(keycode)%4)*90));
                    disableMovement = true;
                    var player = document.getElementById(Math.floor(viewDiameter/2)+","+Math.floor(viewDiameter/2)).querySelector(".Creature");
                    var offset = player.getBoundingClientRect().width;
                    var direction  = [offset*Math.cos((keyBinds["Movement"].indexOf(keycode)%4)*90*Math.PI/180),offset*Math.sin((keyBinds["Movement"].indexOf(keycode)%4)*90*Math.PI/180)];
            ...
            document.querySelectorAll('button').forEach((Item) => {
                Item.addEventListener("click", () => {
                    if (Item.dataset.direction && keyBinds["Movement"].includes(Item.dataset.direction)){
